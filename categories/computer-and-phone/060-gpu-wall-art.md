...
- [ ] LED strip — RGB preferred for color effects *(~$8, electronics supplier)*
- [ ] Standoff bolts or spacers — to mount cards with a gap for backlighting *(hardware store)*
- [ ] Machine screws + nuts — matching your standoffs *(hardware store)*
...
3. **Paint or finish the backing board.** Matte black works best for contrast. Paint or cover with black fabric. The dark background makes the green/blue/red PCBs and the LED glow pop dramatically.
4. **Install LED strips.** Run RGB LED strips around the perimeter of the backing board, facing inward, about 1 inch from the edge. You can also add strips between mounting positions so each card gets its own backlight halo.
5. **Mount the standoffs.** Drill holes at your marked positions and install standoff spacers (15-25mm height works well). The gap between the card and the backing board is what creates the backlight effect — LEDs shine through the gap and around the edges.
6. **Attach the GPUs.** Use the cards' existing mounting holes (PCI bracket screw holes, heatsink mounting holes) to bolt them to the standoffs. If holes don't line up, drill new mounting holes through non-critical areas of the PCB.
7. **Wire the LEDs.** Connect all LED strips to a single controller and power supply. Mount the controller and power supply behind the backing board. Route the power cable to exit cleanly at one edge.
