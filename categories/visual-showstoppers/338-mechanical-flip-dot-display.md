...
6. **Build the shift register driver circuit.** Chain 32 shift registers (74HC595) in series — the Arduino clocks out 256 bits of data that propagate down the chain. Each shift register output drives an H-bridge channel that controls one coil. The H-bridge allows you to pulse each coil in either direction (flip to yellow or flip to black). Build this on perfboard or order a small PCB run. Test each channel before assembling the full grid.
